...
difference() {    
    base_top_plate();
    key_switches();
    unit_cube(-8 * w_unit - padding_x_l, y1 + w_unit);
    unit_cube(-8 * w_unit - padding_x_l, y1);
    unit_cube(-(2.25 + 6) * w_unit - padding_x_l, y3);
    unit_cube(-(2.75 + 5) * w_unit - padding_x_l, y4);
    unit_cube(-(2.75 + 5) * w_unit - padding_x_l, y5);
    unit_cube(-(2.75 + 5) * w_unit - padding_x_l, y5 - w_unit);
}

module base_top_plate() {
    plate_w_x = padding_x_l + w_unit / 2 + 8 * w_unit + padding_x_r;
    plate_w_y = padding_y_t + w_unit / 2 + 4.5 * w_unit + padding_y_b;
    plate_h = 4;

    translate([-plate_w_x + w_unit / 2 + padding_x_r, -plate_w_y + padding_y_t + w_unit / 2, -plate_h]) {
